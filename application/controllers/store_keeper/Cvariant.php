...
class Cvariant extends CI_Controller
{
    ...
    {
        parent::__construct();
        $this->load->library('store_keeper/lvariant');
        $this->load->model('store_keeper/Variants');
    ...
    public function insert_variant()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            foreach ($category_ids as $category_id):
                $this->db->query("INSERT INTO `category_variant` (`category_id`, `variant_id`, `created_at`, `updated_at`) VALUES ('".$category_id."', '".$variant_id."', now(), now())");
            endforeach;
            if ($result == TRUE) {
    ...
    public function variant_update($variant_id = null)
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $this->db->query("delete from category_variant where variant_id='".$variant_id."'");
            foreach ($category_ids as $category_id):
                $this->db->query("INSERT INTO `category_variant` (`category_id`, `variant_id`, `created_at`, `updated_at`) VALUES ('".$category_id."', '".$variant_id."', now(), now())");
            endforeach;
