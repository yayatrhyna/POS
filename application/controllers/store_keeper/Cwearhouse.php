...
class Cwearhouse extends CI_Controller {
	...
	function __construct() {
      parent::__construct();
		$this->load->library('store_keeper/lwearhouse');
		$this->load->model('store_keeper/Wearhouses');
		$this->load->model('store_keeper/Stores');
	...
	public function insert_wearhouse()
	{
        ...
        else
        {
			...
			$data=array(
				...
				'wearhouse_name' 	=> $this->input->post('wearhouse_name'),
				'wearhouse_address' => $this->input->post('wearhouse_address'),
	...
	public function insert_wearhouse_product(){
        ...
        else
        {
			...
			$data=array(
				...
				'wearhouse_id' 		   => $this->input->post('wearhouse_name'),
	...
	public function insert_wearhouse_transfer(){
		...
		if ($transfer_to == 1) {
			...
			$data = array(
				...
				't_warehouse_id'  => $this->input->post('t_warehouse'), 
				'status'	   => 4,
			...
			$data1 = array(
				'transfer_id'  => $this->auth->generator(15),
				'warehouse_id' => $this->input->post('t_warehouse'),
				'product_id'   => $this->input->post('product_id'),
				'variant_id'   => $this->input->post('variant_id'),
				'quantity' 	   => $quantity,
				'transfer_by'  => $this->session->userdata('user_id'), 
				't_warehouse_id'  => $this->input->post('wearhouse_id'), 
				'date_time'    => date("m-d-Y"),
			);
