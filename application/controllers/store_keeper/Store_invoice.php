...
class Store_invoice extends CI_Controller
{
	...
	{
		parent::__construct();
		$this->load->library('store_keeper/lstore_invoice');
		$this->load->model('store_keeper/Store_invoices');
	...
	{
		$this->load->library('store_keeper/linvoice');
		$content = $this->linvoice->pos_invoice_html_data_redirect($invoice_id);
	}
		...
		if ($stok_report > 0) {
			...
			if ($product_details['variant_id']) {
				...
				foreach ($exploded as $elem) {
					...
					if ($product_details['variant_id'] == $result->variant_id) {
	...
	// new one
	public function insert_pos_invoice2()
	{
		$product_id = $this->input->post('product_id');
		...
		if ($stok_report > 0) {
			...
			$product_details = $this->Store_invoices->get_total_product2($product_id, $store_id);
			$html = "";
			if ($product_details['variant_id']) {
				$exploded = explode(',', $product_details['variant_id']);
				$html .= "<option>Select Variant</option>";
			...
			if ($product_details['total_product'] > 0) {
                ...
                class=\"form-control text-right available_quantity_" . $product_id . "\" value=\"0\" readonly=\"readonly\" style=\"width: 100px\"/>
			            ...
			            <td scope=\"row\">
			                <input type=\"number\" name=\"product_quantity[]\" onkeyup=\"quantity_calculate('" . $product_id . "');\" onchange=\"quantity_calculate('" . $product_id . "');\" id=\"total_qntt_" . $product_id . "\" class=\"form-control text-right\" value=\"1\" min=\"1\" style=\"width:100px\"/>
			            </td>
			            <td width=\"\">
