...
//        dd($rate);
		...
		$content = $this->lcategory->category_product($cat_id,$links,$config["per_page"],$page,$price_range,$size,
            $brand,$rate);
		$this->template->full_website_html_view($content);
	}	
    ...
    public function category_product_search()
    {
        ...
        $content        = $this->lcategory->category_product_search($product_name);
        $this->template->full_website_html_view($content);
    }	
    ...
    public function brand_search(){
        ...
        $brand_url_ids  = $this->input->post('brand_url_ids');
