...
class Lhome {
	...
	public function home_page()
	{
        ...
        $HomeForm = $CI->parser->parse('website/themes/'.$theme.'/home',$data,true);
		return $HomeForm;
	}	
	...
	public function checkout()
	{
		...
		$sslcommerz_status 		= $CI->Homes->payment_status('4');

		$country_id = $CI->session->userdata('country');
		if ($country_id) {
			$state_list 		= $CI->Homes->select_state_country();
		}else{
            $state_list 		= $CI->Homes->get_country_state(18); //bangladesh
        }
		...
		$data = array(
				...
				'sslcommerz_status' 	=> (!empty($sslcommerz_status)?$sslcommerz_status->status:0),

			);
		$checkout = $CI->parser->parse('website/themes/'.$theme.'/checkout',$data,true);
		return $checkout;
	}	
	...
	public function view_cart()
	{
		...
		$viewCart = $CI->parser->parse('website/themes/'.$theme.'/cart',$data,true);
		return $viewCart;
	}
	...
	public function order_html_data($order_id)
	{
	    ...
	    if (!empty($data['customer_email']) && $server_status) {
