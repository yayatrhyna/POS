...
class Lunit {
	...
	{
		$CI =& get_instance();
		$CI->load->model('store_keeper/Units');
		$data = array(
				'title' => display('add_unit')
			);
		$customerForm = $CI->parser->parse('unit/add_unit',$data,true);
		return $customerForm;
	}
	...
	public function unit_list()
	{
		...
		$customerList = $CI->parser->parse('unit/unit',$data,true);
		return $customerList;
	}
	...
	public function unit_edit_data($unit_id)
	{
		...
		$chapterList = $CI->parser->parse('unit/edit_unit',$data,true);
		return $chapterList;
	}
}
?>