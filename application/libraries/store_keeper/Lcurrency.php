...
class Lcurrency {
	...
	{
		$CI =& get_instance();
		$CI->load->model('store_keeper/Currencies');
		$data = array(
				'title' => display('add_currency')
			);
		$customerForm = $CI->parser->parse('currency/add_currency',$data,true);
		return $customerForm;
	}
	...
	public function currency_list()
	{
		...
		$customerList = $CI->parser->parse('currency/currency',$data,true);
		return $customerList;
	}
	...
	public function currency_edit_data($currency_id)
	{
		...
		$chapterList = $CI->parser->parse('currency/edit_currency',$data,true);
		return $chapterList;
	}
}
?>