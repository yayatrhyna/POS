...
class Lcoupon {
	...
	{
		$CI =& get_instance();
		$CI->load->model('store_keeper/Coupons');

		$data = array(
				'title' 	=> display('add_coupon'),
			);

		$couponForm = $CI->parser->parse('coupon/add_coupon',$data,true);
		return $couponForm;
	}
	...
	public function coupon_edit_data($coupon_id)
	{
		...
		$couponEdit = $CI->parser->parse('coupon/edit_coupon',$data,true);
		return $couponEdit;
	}
}
?>