...
<div class="content-wrapper">
		            ...
		            <div class="panel-body"> 
                        ...
                        <?php if (!isset($from_date)) { ?>
                          <div class="form-group">
                              ...
                              <input type="text" onchange="export_from_date(event);" name="from_date" class="form-control datepicker" id="from_date" placeholder="<?php echo display('start_date') ?>" autocomplete="off">
                          </div>

                          <div class="form-group">
                              <label class="" for="to_date"><?php echo display('end_date') ?></label>
                              <input type="text" onchange="export_to_date(event);" name="to_date" class="form-control datepicker" id="to_date" placeholder="<?php echo display('end_date') ?>" value="<?php echo $today?>" autocomplete="off">
                          </div>
                        <?php } else { ?>
                          <div class="form-group">
                              <label class="" for="from_date"><?php echo display('start_date') ?></label>
                              <input type="text" onchange="export_from_date(event);" name="from_date" class="form-control datepicker" id="from_date" placeholder="<?php echo display('start_date') ?>" value="<?php echo $from_date?>" autocomplete="off">
                          </div> 

                          <div class="form-group">
                              <label class="" for="to_date"><?php echo display('end_date') ?></label>
                              <input type="text" onchange="export_to_date(event);" name="to_date" class="form-control datepicker" id="to_date" placeholder="<?php echo display('end_date') ?>" value="<?php echo $to_date?>" autocomplete="off">
                          </div>
                        <?php } ?>
		               ...
		               <?php echo form_close()?>
                   <br>
                   <form action="<?php echo base_url('Admin_dashboard/export_invoice') ?>" method="post">
                    <input type="hidden" name="export_from_date" id="export_from_date">
                    <input type="hidden" name="export_to_date" id="export_to_date" value="<?php echo $today?>">
                    <button type="submit" class="btn btn-primary m-b-5 m-r-2"><i class="ti-align-justify"> </i> Export Detail</button>
                    </form>

					<form action="<?php echo base_url('Admin_dashboard/export_invoice_summary') ?>" method="post">
                    <input type="hidden" name="export_summary_from_date" id="export_summary_from_date">
                    <input type="hidden" name="export_summary_to_date" id="export_summary_to_date" value="<?php echo $today?>">
					<button type="submit" class="btn btn-primary m-b-5 m-r-2"><i class="ti-align-justify"> </i> Export Summary</button>
                    </form>
 ...
 <!-- Sales Report End -->
 <script>
  function export_from_date(e){
    $('#export_from_date').val(e.target.value);
	$('#export_summary_from_date').val(e.target.value);
  }
  function export_to_date(e){
    $('#export_to_date').val(e.target.value);
	$('#export_summary_to_date').val(e.target.value);
  }
 </script>