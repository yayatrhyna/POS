		                ...
		                <div class="table-responsive">
		                    <table id="dataTableExample2" class="table table-bordered table-striped table-hover">
								...
								<?php  if ($customers_list) { ?>
									...
									<tr>
										<td><?php echo $single_list['sl']?></td>
										<td>
											<?php if ($this->session->userdata('user_type') == '4') { ?>
												<?php echo $single_list['customer_name']?>
											<?php }else{ ?>		
											<a href="<?php echo base_url().'Ccustomer/customerledger/'.$single_list['customer_id']; ?>"> <?php echo $single_list['customer_name'];?> <i class="fa fa-user pull-right" aria-hidden="true"></i></a>
											<?php } ?>	
										</td>
                                        <?php if(!empty($single_list['customer_short_address'])){?>
										<td><?php echo $single_list['customer_short_address']?></td>
                                        <?php }else{?>
                                            <td><?php echo $single_list['customer_address_1']?></td>
                                        <?php }?>
										<td><?php echo $single_list['customer_mobile'];?></td>
										<td><?php echo $single_list['customer_email'];?></td>
										<td>
											<center>
											<?php echo form_open()?>
												<a href="<?php echo base_url().'Ccustomer/customer_update_form/'.$single_list['customer_id']; ?>" class="btn btn-info btn-sm" data-toggle="tooltip" data-placement="left" title="<?php echo display('update') ?>"><i class="fa fa-pencil" aria-hidden="true"></i></a>
											<?php if(1 != $single_list['customer_id']){?>
												<a href="<?php echo base_url('Ccustomer/customer_delete/').$single_list['customer_id'];?>" class="btn btn-danger btn-sm" onclick="return confirm('<?php echo display('are_you_sure_want_to_delete')?>');" data-toggle="tooltip" data-placement="right" title="" data-original-title="<?php echo display('delete') ?> "><i class="fa fa-trash-o" aria-hidden="true"></i></a>
