...
// ====================STORE WISE STOCK REPORT =======================
    ...
    public function individual_store_wise_product($per_page = NULL, $page = NULL)
    {
        ...
        foreach ($result as $product) {
            ...
            $sql = 'select sum(a.quantity) as sell from invoice_details a  where store_id="' . $store_id . '" and product_id="' . $product->product_id . '" and variant_id="' . $product->variant_id . '"';
            $product_sell = $this->db->query($sql)->row();
            $test[] = array_merge((array)$product, (array)$product_sell);
        }
...
//=============sales report store wise=================
    ...
    {
        $today = date('m-d-Y');
        $this->db->select("a.*,b.customer_id,b.customer_name,c.*");
        $this->db->from('invoice a');
    ...
    //Retrieve todays_sales_report
    public function todays_sales_report_by_date($per_page, $page, $from_date, $to_date, $payment)
    {
        if ($from_date == null) {
        ...
        $this->db->select("a.*,b.customer_id,b.customer_name,c.invoice_id,c.card_type,d.*");
        $this->db->from('invoice a');
        $this->db->join('customer_information b', 'b.customer_id = a.customer_id');
        $this->db->join('cardpayment c', 'c.invoice_id = a.invoice_id');
        $this->db->join('store_set d', 'd.store_id = a.store_id');
        $this->db->where('a.date >=', $from_date);
        $this->db->where('a.date <=', $to_date);
        if ($payment != "All") {
            $this->db->where('c.card_type', $payment);
        }
        $this->db->limit($per_page, $page);
        $this->db->order_by('a.invoice_id', 'desc');
        $query = $this->db->get();
    ...
    public function todays_all_sales_report_by_payment($payment)
    {
        $today = date('m-d-Y');
        $this->db->select("b.card_type, a.invoice_id, a.date, SUM(a.total_amount) AS total_by_payment");
        $this->db->from('invoice a');
        $this->db->join('cardpayment b', 'b.invoice_id = a.invoice_id');
        $this->db->where('b.card_type', $payment);
        $this->db->where('a.date', $today);
        $query = $this->db->get();
        return $query->result_array();
    }
    ...
    public function todays_all_sales_report_by_payment_and_date($payment, $from_date, $to_date)
    {
        $this->db->select("b.card_type, a.invoice_id, a.date, SUM(a.total_amount) AS total_by_payment");
        $this->db->from('invoice a');
        $this->db->join('cardpayment b', 'b.invoice_id = a.invoice_id');
    ...
    public function todays_total_invoice_report_by_payment($payment)
    {
        $today = date('m-d-Y');
        $this->db->select("b.card_type, a.invoice_id, a.date");
        $this->db->from('invoice a');
        $this->db->join('cardpayment b', 'b.invoice_id = a.invoice_id');
        $this->db->where('b.card_type', $payment);
        $this->db->where('a.date', $today);
    ...
    public function todays_total_invoice_report_by_payment_and_date($payment, $from_date, $to_date)
    {
        $this->db->select("b.card_type, a.invoice_id, a.date");
        $this->db->from('invoice a');
        $this->db->join('cardpayment b', 'b.invoice_id = a.invoice_id');
    ...
    public function todays_all_sales_report()
    {
        $today = date('m-d-Y');
    ...
    public function todays_all_sales_report_by_date($from_date, $to_date)
    {
        if ($from_date == null) {
          $from_date = $this->session->userdata('report_sales_from_date');
        }
    ...
    public function todays_sales_report_count_by_date($from_date, $to_date, $payment)
    {
        if ($from_date == null) {
          $from_date = $this->session->userdata('report_sales_from_date');
        }
        ...
        $this->db->select("a.*,b.customer_id,b.customer_name, c.invoice_id, c.card_type");
        $this->db->from('invoice a');
        $this->db->join('customer_information b', 'b.customer_id = a.customer_id');
        $this->db->join('cardpayment c', 'c.invoice_id = a.invoice_id');
        $this->db->where('a.date >=', $from_date);
