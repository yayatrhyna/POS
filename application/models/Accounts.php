...
class Accounts extends CI_Model {
    ...
    public function daily_closing_entry( $data ){
        ...
        $closing_data=$this->db->select('date')->from('daily_closing')->where('date',$data['date'])->get()->row();
    ...
    public function accounts_closing_data()
    {
      ...
      $cash_in=  ($this->cash_data(2)+$last_closing_amount[0]['cash_in']);
      $cash_out=  ($this->cash_data(1) +$last_closing_amount[0]['cash_out']);
