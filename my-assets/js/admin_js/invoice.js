...
function  quantity_limit(item) {
    ...
    if( parseInt(available_quantity) < parseInt(current_quantity)){
        ...
        current_quantity = 1;
        $("#total_qntt_" + item).val(current_quantity)
...
function quantity_calculate(item) {
    ...
    if (isNaN(quantity)) 
    {
      alert("Qty must numbers");
      quantity = 1;
      $("#total_qntt_" + item).val(quantity)
      // return false;
    }

    // total product_quantity
    var total_quantity = 0;
    var arr = $('input[name="product_quantity[]"]').map(function () {
        var qty = parseInt(this.value);
        if (isNaN(qty)) {
            qty = 0;
        }
        return total_quantity += qty; // $(this).val()
    }).get();

    var format_total_quantity = total_quantity.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
    $("#total_qty_product").val(format_total_quantity)
...
function calculateSum() {
    ...
    sum = +cgst+ +sgst+ +igst+ +e+ - f+ - inv_dis+ +shipping_charge;


    var service = (ser_chg/100) * sum;
    var total = service + sum;
        // alert('dd');
        // console.log(total);
    $("#grandTotal").val(total.toFixed(2));
    $(".total_bill").text(total.toFixed(2));
...
function deletePosRow(t) {
    var quantity = $("#total_qntt_"+t).val();
    var total_quantity = $("#total_qty_product").val();
    ...
    var new_total_quantity = total_quantity-quantity;
    $("#total_qty_product").val(new_total_quantity);
